...
bool PTU::halt()
{
  ROS_INFO("Sending command to stop PTU.");
  
  if (!initialized()) return false;
  
  std::string buffer = sendCommand("h ");
  
  if (buffer.empty() || buffer[0] != '*')
  {
    ROS_ERROR("Error PTU halt command response not received.");
    return false;
  }
  
  ROS_INFO("PTU stop command response received.");
  return true;
}
...
// get abusolute position
int PTU::getIntPosition(char type)
{
  ...
  return parseResponse<double>(buffer);
}

// get position in radians
float PTU::getPosition(char type)
{
  return getIntPosition(type) * getResolution(type);
}
...
bool PTU::setPosition(char type, float pos, bool block)
{
  ...
  if (block)
  {
    while (getIntPosition(type) != count)
    {
      usleep(1000);
    }
  }
...
// set base speed in radians/sec
bool PTU::setBaseSpeed(char type, float pos)
{
  if (!initialized()) return false;
  ...
  std::string buffer = sendCommand(std::string() + type + "b" +
                                   lexical_cast<std::string>(count) + " ");
...
// get base speed in radians/sec
float PTU::getBaseSpeed(char type)
{
  if (!initialized()) return -1;
  ...
  if (buffer.length() < 3 || buffer[0] != '*')
  {
    ROS_ERROR("Error getting pan-tilt base speed");
    return -1;
  }
...
bool PTU::setSpeedMode(char type)
{
  if (!initialized()) return false;
...
// set immediate position execution mode
bool PTU::setImmediatePosition()
{
  if (!initialized()) return false;
  
  std::string buffer = sendCommand("i ");
  
  if (buffer.empty() || buffer[0] != '*')
  {
    ROS_ERROR("Error setting pan-tilt immediate position execution mode");
    return false;
  }
...
// set await position command completion mode
bool PTU::setAwaitPosition()
{
  if (!initialized()) return false;
  
  std::string buffer = sendCommand("a ");
  
  if (buffer.empty() || buffer[0] != '*')
  {
    ROS_ERROR("Error setting pan-tilt await position command completion mode");
    return false;
  }
