#ifndef NDT_OMP_IMPL_H
#define NDT_OMP_IMPL_H
    ...
    bool compute_hessian)
{
  ...
  std::vector<double> scores(n_max_threads);
  std::vector<Eigen::Matrix<double, 6, 1>, Eigen::aligned_allocator<Eigen::Matrix<double, 6, 1>>> score_gradients(n_max_threads);
  std::vector<Eigen::Matrix<double, 6, 6>, Eigen::aligned_allocator<Eigen::Matrix<double, 6, 6>>> hessians(n_max_threads);
  ...
  std::vector<std::vector<TargetGridLeafConstPtr>> neighborhoods(n_max_threads);
  std::vector<std::vector<float>> distancess(n_max_threads);
