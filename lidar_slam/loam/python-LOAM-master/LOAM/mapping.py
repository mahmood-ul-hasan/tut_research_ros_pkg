...
pc_pub = rospy.Publisher('aligned_pointcloud', PointCloud2, queue_size=10)
...
class Mapper:
    ...
    def append_undistorted(self, pcd, T, edge_points, surface_points, vis=False):
        ...
        if not self.init:
            ...
            # Convert aligned_pcds to PointCloud2 and publish
            for pcd in self.aligned_pcds:
                header = Header()
                header.stamp = rospy.Time.now()
                header.frame_id = 'map'  # Replace with your desired frame
                ros_cloud = pc2.create_cloud_xyz32(header, np.asarray(pcd.points))
                pc_pub.publish(ros_cloud)
