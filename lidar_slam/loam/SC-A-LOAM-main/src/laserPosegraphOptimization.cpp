...
std::optional<gtsam::Pose3> doICPVirtualRelative( int _loop_kf_idx, int _curr_kf_idx )
{
    ...
    float loopFitnessScoreThreshold = 0.7; // user parameter but fixed low value is safe. 
...
int main(int argc, char **argv)
{
    ...
    // save directories 
	nh.param<std::string>("save_directory", save_directory, "/media/aisl2/aisl_data/catkin_ws/src/lidar_slam/loam/SC-A-LOAM-main/saved_data/"); // pose assignment every k m move 
	...
	nh.param<double>("sc_max_radius", scMaximumRadius, 300.0); // 80 is recommended for outdoor, and lower (ex, 20, 40) values are recommended for indoor 
...
ros::Subscriber subLaserCloudFullRes = nh.subscribe<sensor_msgs::PointCloud2>("/velodyne_points_filtered", 100, laserCloudFullResHandler);
	ros::Subscriber subLaserOdometry = nh.subscribe<nav_msgs::Odometry>("/odom_world_to_velodyne", 100, laserOdometryHandler);
