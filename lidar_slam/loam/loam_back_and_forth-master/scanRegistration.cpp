...
// Get the current point cloud time ( the timestamp in ROS is generally: MsgPtr->header.stamp.toSec()
  ...
  // gazebo
  // laserAngleCur = atan2(laserCloud->points[cloudSize - 1].x , 
  //                       laserCloud->points[cloudSize - 1].y );
// // LOAM
  laserAngleCur = atan2(laserCloud->points[cloudSize - 1].y - laserCloud->points[0].y, 
     ...
     laserAngleCur = round(laserAngleCur);
...
//  int cloudSize = laserCloudIn->points.size();
    ...
    std::cout << "Points ------------> : " << laserCloudExtreCur2.width  << " " << laserCloudLast2.width  << " time: " << diff_sweep_time << " count: " << counter_sweep << "================================================================================================================" << std::endl;
     std::cout << "Sweep No: " << counter_sweep <<  " is completed "<<std::endl;
    std::cout << " Last: " << laserAngleLast << " Cur: " << laserAngleCur << " laserRotDir: " << laserRotDir <<std::endl;

    //  std::cout << " -----------------------------------" <<std::endl;
     std::cout << "            " <<std::endl;

    // std::cout << "  laserCloudLast2    : " << laserCloudLast2.width << std::endl;
