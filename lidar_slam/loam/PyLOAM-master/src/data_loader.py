...
class PCDLoader(DataLoader):
    ...
    def get_pc(self, index):
        if index < self.scan_num:
            # Read the binary .bin file directly
            file_path = os.path.join(self.path, self.file_list[index])
            pcd = np.fromfile(file_path, dtype=np.float32).reshape(-1, 4)[:, :3]  # Assumes 4 values per point (x, y, z, intensity)
            return pcd
        else:
            print("Access out of range!")
            return None
