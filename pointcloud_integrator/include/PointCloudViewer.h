...
public:
	...
	void addFrameMsg(pointcloud_integrator::keyScanMsgConstPtr msg);
	void addGraphMsg(pointcloud_integrator::keyScanGraphMsgConstPtr msg);
	//void setIntrinsicParameters(sensor_msgs::CameraInfoConstPtr msg);
	void setIntrinsicParameters();
	//void setExtrinsicParameters(geometry_msgs::PoseConstPtr msg);  //TODO: couldn't pass PoseConstPtr to this method... should look it up how to deal with the problem.
	void setExtrinsicParameters(const geometry_msgs::Pose& msg);
	bool hasIntrinsicParams(void);
	std::vector<std::string> split(const std::string& input, char delimiter);
	bool getIntrinsicParams(int& height, int& width, float K[9]);
	void snapshot();
	
	cv_bridge::CvImage snapshot_;
...
private:
  ...
  // Camera parameters
  bool hasIntrinsicParams_ = false;
