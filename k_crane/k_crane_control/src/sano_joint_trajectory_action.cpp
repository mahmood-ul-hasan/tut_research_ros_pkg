...
class Crane {
	public:
		...
		control_msgs::FollowJointTrajectoryGoal crane_extension_trajectory();
...
control_msgs::FollowJointTrajectoryGoal Crane::crane_extension_trajectory(){
	// Read CSV file
	CSV::CsvFile angle_data("/home/harumo/catkin_ws/src/k_crane/data/IMU_experiment/input_data/nonlinear_input_position.csv");
	...
	for (int i = 0; i < angle_data.collumn_size(); i++) {
		...
		goal.trajectory.points[i].positions[0] = 60 * M_PI/180;						// pitch joint
...
int main(int argc, char* argv[])
{
	...
	// service client
	std_srvs::Empty unpaused_srv;
	ros::ServiceClient unpause_client = node_handle.serviceClient<std_srvs::Empty>("gazebo/unpause_physics");
	...
	auto trajectory_msgs = crane.crane_extension_trajectory();
	...
	////unpause
	//if (unpause_client.call(unpaused_srv)) {
	//	ROS_INFO_STREAM(unpaused_srv.response);
	//}
	//else {
	//	ROS_INFO_STREAM(unpaused_srv.response);
	//}
