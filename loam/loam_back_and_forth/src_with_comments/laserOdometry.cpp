...
1. Calculation of Jacobian For the loss
-----------------------------------
function (loss is the distance calculated by the feature point, the code usesBBBmatrix
representation): l o s s = F ( X ~ k + 1 , T k + 1 ) loss=F(\tilde{X}_{k+1},T_{k+1}) l o s s=F ( 
x ~    k + 1 ​  ,T  k + 1 ​  )  Its Jacobian matrix (used in the codeAAAAMatrix
representation) is: J = ∂ F ∂ T k + 1 = ∂ F ∂ X ~ k + 1 ∂ X ~ k + 1 ∂ T k + 1 J=\frac{\partial
