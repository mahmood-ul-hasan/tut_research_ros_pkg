...
void laserCloudHandler(const sensor_msgs::PointCloud2ConstPtr& laserCloudIn2)
{
  ...
  *laserCloudLessExtreCur += *surfPointsLessFlatDS;
  ...
  if (skipFrameCount >= skipFrameNum) {
    ...
    ROS_INFO ("imu rotate = %f %f %f", imuShiftFromStartXCur, imuShiftFromStartYCur, imuShiftFromStartZCur);
    ROS_INFO ("imu Velo = %f %f %f", imuVeloFromStartXCur, imuVeloFromStartXCur, imuVeloFromStartXCur);
