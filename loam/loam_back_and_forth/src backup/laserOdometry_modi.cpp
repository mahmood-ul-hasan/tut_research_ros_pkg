...
//Relative transfer relative to the previous frame and the first frame

//The state transition amount of the current frame relative to the previous frame, in the local frame
float transform[6] = {0};
...
//The distortion displacement of the last point of the point cloud relative to the first point due to acceleration and deceleration
...
// Accept large curvature points, convert ros point cloud format to pcl format and remove empty points:
...
void laserCloudLastHandler(const sensor_msgs::PointCloud2ConstPtr& laserCloudLast2)
{
  ...
  if (ros::Time::now().toSec() > timeLaserCloudLast + 0.005) {
    timeLaserCloudLast = ros::Time::now().toSec();
...
int main(int argc, char** argv)
{
  ...
  // tf::StampedTransform static_camera_laser_Trans;
  // static_camera_laser_Trans.frame_id_ = "camera";
  // static_camera_laser_Trans.child_frame_id_ = "front_laser_link";
  ...
  while (status) {
    ...
    if (newLaserPoints) {
      ...
      laserOdometryTrans.stamp_ = ros::Time().fromSec(timeLaserCloudLast);
      current_time = timeLaserCloudLast;
      ...
      // geometry_msgs::Quaternion static_Quat = tf::createQuaternionMsgFromRollPitchYaw( 0, 90*PI/180,0);
      // static_camera_laser_Trans.stamp_ = ros::Time().fromSec(timeLaserCloudLast);
      // static_camera_laser_Trans.setRotation(tf::Quaternion(static_Quat.x, static_Quat.y, static_Quat.z, static_Quat.w));
      // static_camera_laser_Trans.setOrigin(tf::Vector3(0, 0, 0));
      // tfBroadcaster.sendTransform(static_camera_laser_Trans);


      // static_imu_laser_Trans.stamp_ = ros::Time().fromSec(timeLaserCloudLast);
      // static_imu_laser_Trans.setRotation(tf::Quaternion(0,0,0,1));
      // static_imu_laser_Trans.setOrigin(tf::Vector3(0, 0, 0));
      // tfBroadcaster.sendTransform(static_imu_laser_Trans);
