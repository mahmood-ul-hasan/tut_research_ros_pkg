...
int main(int argc, char **argv)
{
  ...
  ros::init(argc, argv, "ld_lrs3611_node");
  ros::NodeHandle nh;
  ros::NodeHandle n("~");
  ...
  while (ros::ok())
  {
    ...
    // ROS_INFO_STREAM("cfg.scaningFrequency is: " << cfg.scaningFrequency);
    // if (cfg.scaningFrequency != 2500) // for lms1xx
    ROS_INFO_STREAM("cfg.scaningFrequency: " << cfg.scaningFrequency);
    if (cfg.scaningFrequency < 500 || cfg.scaningFrequency > 1500) // for ld-lrs36xx
    {
      laser.disconnect();
    ...
    while (ros::ok())
    {
      ...
      if (laser.getScanData(&data))
      {
        ...
        {
          // scan_msg.ranges[i] = data.dist1[i] * 0.001; // for lms1xx
          scan_msg.ranges[i] = data.dist1[i] * 0.001 * 4; // for ld-lrs36xx
        }
  ...
  // laser.scanContinous(0); laser.stopMeas(); // for lms1xx
  laser.stopMesu4Ldlrs(); // for ld-lrs36xx
  laser.disconnect();
