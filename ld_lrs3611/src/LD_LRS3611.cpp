...
#include "LD_LRS3611/LD_LRS3611.h"
#include "console_bridge/console.h"

LD_LRS3611::LD_LRS3611() : connected_(false)
{
}

LD_LRS3611::~LD_LRS3611()
{
}

void LD_LRS3611::connect(std::string host, int port)
{
  if (!connected_)
  {
...
bool LD_LRS3611::isConnected()
{
  return connected_;
}

void LD_LRS3611::startMeas()
{
  char buf[100];
...
status_t LD_LRS3611::queryStatus()
{
  char buf[100];
...
scanCfg LD_LRS3611::getScanCfg() const
{
  scanCfg cfg;
  char buf[100];
...
void LD_LRS3611::setScanCfg(const scanCfg &cfg)
{
  char buf[100];
...
void LD_LRS3611::setScanDataCfg(const scanDataCfg &cfg)
{
  char buf[100];
...
scanOutputRange LD_LRS3611::getScanOutputRange() const
{
...
void LD_LRS3611::scanContinous(int start)
{
  char buf[100];
...
bool LD_LRS3611::getScanData(scanData* scan_data)
{
  fd_set rfds;
  FD_ZERO(&rfds);
...
void LD_LRS3611::parseScanData(char* buffer, scanData* data)
{
  ...
  for (int i = 0; i < NumberChannels16Bit; i++)
  {
    ...
    // ROS_DEBUG("ScalingFactor(1) : %s", tok);
    // std::cout << "ScalingFactor(1) : " << tok << std::endl;
    tok = strtok(NULL, " "); //ScalingOffset
    // ROS_DEBUG("ScalingOffset(1) : %s", tok);
    // std::cout << "ScalingOffset(1) : " << tok << std::endl;
  ...
  for (int i = 0; i < NumberChannels8Bit; i++)
  {
    ...
    tok = strtok(NULL, " "); //ScalingFactor
    // ROS_DEBUG("ScalingFactor(2) : %s", tok);
    // std::cout << "ScalingFactor(2) : " << tok << std::endl;
    tok = strtok(NULL, " "); //ScalingOffset
    // ROS_DEBUG("ScalingOffset(2) : %s", tok);
    // std::cout << "ScalingOffset(2) : " << tok << std::endl;
    tok = strtok(NULL, " "); //Starting angle
    tok = strtok(NULL, " "); //Angular step width
    tok = strtok(NULL, " "); //NumberData
    int NumberData;
...
// added method
void LD_LRS3611::stopMesu4Ldlrs()
{
  char buf[100];
  sprintf(buf, "%c%s%c", 0x02, "sMI 0 3 F4724744", 0x03);
  write(socket_fd_, buf, strlen(buf));
  char buf1[100];
  sprintf(buf1, "%c%s%c", 0x02, "sMI 38", 0x03);
  write(socket_fd_, buf1, strlen(buf1));
  int len = read(socket_fd_, buf, 100);
  write(socket_fd_, buf, strlen(buf));
...
void LD_LRS3611::setAutostartMeasurement(/*bool isSetAuto*/)
{
  char buf[100];
  sprintf(buf, "%c%s %d%c", 0x02, "sWN LMPautostartmeas", 1, 0x03);
  //sprintf(buf, "%c%s %d%c", 0x02, "sWN LMPautostartmeas", (isSetAuto ? 1 : 0), 0x03);
