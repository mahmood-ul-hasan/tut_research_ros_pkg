...
seg.setRadiusLimits(0.0, 8.0);
    ...
    *cloud_remaining = *cloud_non_walls;
    ...
    ros::NodeHandle nh;
    ros::Publisher pub_cloud_filtered = nh.advertise<PointCloud>("cloud_objects", 10, true);

    // for (int i = 0; i < 5; i++) {
    //     pub_cloud_filtered.publish(cloud_remaining);
    //     ros::spinOnce();
    //     ros::Duration(0.070).sleep(); // Wait for 1 second
    // }

    for (int i = 0; i <= 5; i++) {
    //  if(pub_cloud_filtered.getNumSubscribers()) {
        pub_cloud_filtered.publish(cloud_remaining);
    //   }
    }
...
for (auto indices : cluster_indices)
{
    ...
    ros::Publisher pub_cluster = nh.advertise<PointCloud>(topic_name, 10, true);
  ...
  // Publish cluster points
    ...
    std::cout << " end ====================================================================== " << wall_count << std::endl;
...
int main(int argc, char** argv)
{
    ...
    ros::Subscriber sub_cloud = nh.subscribe<PointCloud>("/laser_pointcloud_assembler", 1, cloudCallback);
