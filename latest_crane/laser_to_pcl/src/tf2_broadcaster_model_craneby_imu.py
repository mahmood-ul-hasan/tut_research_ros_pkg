...
def imu_upper_link_callback(msg):
    ...
    quat_upper_yaw = quaternion_from_euler(0, 0, euler_upper[2]-upper_link_default_joint)
    # quat_upper_yaw = quaternion_from_euler(0, 0, 0)
    t.transform.translation.x = 0.0
    t.transform.translation.y = 0.0
    t.transform.translation.z = 1.46
    ...
    # print("  ",ii,  euler_upper[1]*180/math.pi ,  (euler_upper[2]-upper_link_default_joint)*180/math.pi,  rotating_base)
...
def imu_boom_link_callback(msg):
    ...
    print(math.floor(euler_boom[1]*180/math.pi) ,  math.floor(euler_boom[2]*180/math.pi),  math.floor(rotating_base), j)
    ...
    if flag == True:
        br = tf2_ros.TransformBroadcaster()
        br.sendTransform(transfer_rotating_base)
    ...
    # static_transform_publisher("world", "base_link" , 0.0 , 0.0, 1.2,   0, 0, 0)
    # static_transform_publisher("base_link", "lower_link" , 0.0 , 0.0, 0.28,   0, 0, 0)
    static_transform_publisher("rotation_base_link", "rotation_base_link1",  0, 0, 0,    0*math.pi/180, 90*math.pi/180, 0*math.pi/180)
    static_transform_publisher("boom_link", "rotation_base_link",  -1.15, 0, 0,    0*math.pi/180, 0*math.pi/180, 0*math.pi/180)
    static_transform_publisher("world", "camera_odom_frame",  -1.15, 0, 1.6,    0*math.pi/180, 0*math.pi/180, 0*math.pi/180)
    #static_transform_publisher("base_link", "boom_link", 0, 0 ,0)
    #static_transform_publisher("lower_link", "upper_link", 0.0 , 0.0, 0.4,   0, 0, 0)
    # static_transform_publisher("upper_link", "boom_link")
...
def rotating_base_link_callback(msg):
    ...
    t = geometry_msgs.msg.TransformStamped()
    t.header.stamp = rospy.Time.now()
    t.header.frame_id = "rotation_base_link1"
...
if __name__ == '__main__':
    ...
    pub_rpy_upper_link = rospy.Publisher("/rpy_imu_boom_link", Float32MultiArray, queue_size=1)
