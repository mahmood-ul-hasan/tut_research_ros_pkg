...
def imu_upper_link_callback(msg):
    ...
    odom.child_frame_id = "crane_struct_info"
    odom.pose.pose.orientation.x =array1[0]
    odom.pose.pose.orientation.y =array1[1]
    odom.pose.pose.orientation.z =array1[2]
    pub_rpy_boom_link.publish(odom)
    ...
    quat_upper_yaw = quaternion_from_euler(0, 0, 0* math.pi/180 + euler_upper[2])
...
def imu_boom_link_callback(msg):
    ...
    # broadcast the tf by rpy of imu
    quat_boom_yaw = quaternion_from_euler(0, -(90* math.pi/180 + euler_boom[1]), 0,)
    br = tf2_ros.TransformBroadcaster()
    t = geometry_msgs.msg.TransformStamped()
    t.header.stamp = rospy.Time.now()
    current_time = rospy.Time.now().to_sec()
    ...
    if current_time > pervious_time_boom_link + dt:
        ...
        # static_transform_publisher("world", "camera_init", 0, 0, 0, 0, 0, 0)

        # static_transform_publisher("world", "base_link", 0, 0, 0, 0, 0, 0)
        static_transform_publisher("world", "base_link", 0, 0, 0,   0*math.pi/180, 0, 180*math.pi/180)
        static_transform_publisher("base_link", "lower_link", 0, 0, 1.425, 0, 0, 0)
        static_transform_publisher("boom_link", "velodyne", 16.635 ,0.63, -0.6, -90*math.pi/180, -1.2*math.pi/180, 177*math.pi/180)
        # static_transform_publisher("boom_link", "velodyne", 16.635 ,0.63, 0.82, -90*math.pi/180, -90*math.pi/180, 180*math.pi/180)
        print(math.floor(euler_boom[0] * 180 / math.pi), math.floor(euler_boom[1] * 180 / math.pi), math.floor(euler_boom[2] * 180 / math.pi), j)


        # static_transform_publisher("world", "camera_odom_frame", 0, 0, 0, 0, 0, 0)
        # # static_transform_publisher("world", "base_link", 0, 0, 0, 0, 0, 0)
        # static_transform_publisher("world", "base_link", 0, 0, 0, 0, 0, 180*math.pi/180)
        # static_transform_publisher("base_link", "lower_link", 0, 0, 1.334, 0, 0, 0)
        # static_transform_publisher("boom_link", "laser1", 20.392 ,0.63, 0.82, -90*math.pi/180, 6*math.pi/180, 180*math.pi/180)
        # print(math.floor(euler_boom[0] * 180 / math.pi), math.floor(euler_boom[1] * 180 / math.pi), math.floor(euler_boom[2] * 180 / math.pi), j)
        ...
        # static_transform_publisher("boom_link", "velodyne", 16.635 , -0, -.8, -90*math.pi/180, 1.5*math.pi/180, 180*math.pi/180)
        # # static_transform_publisher("boom_link", "velodyne", 16.635 , 0.63, 0.5, -82*math.pi/180, 90*math.pi/180, -90*math.pi/180)
        # print(math.floor(euler_boom[0] * 180 / math.pi), math.floor(euler_boom[1] * 180 / math.pi), math.floor(euler_boom[2] * 180 / math.pi), j)
...
if __name__ == '__main__':
    ...
    pub_rpy_boom_link = rospy.Publisher("/rpy_imu_boom_link", Odometry, queue_size=10)
    ...
    rospy.Subscriber("/imu_vins/quaternion", QuaternionStamped, imu_boom_link_callback)
    rospy.Subscriber("/imu_vins/quaternion", QuaternionStamped, imu_upper_link_callback)
