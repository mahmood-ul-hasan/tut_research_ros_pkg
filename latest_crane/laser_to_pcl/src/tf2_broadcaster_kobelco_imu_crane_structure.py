...
def boom_rotating_base_callback(msg):
    ...
    rotation_quaternion = quaternion_from_euler(0, 0, 3.14159265359)  # 180 degrees in radians
    ...
    #     initial_yaw_value = euler_upper[2]
    #     print("Initial yaw value set to", initial_yaw_value)

    # if initial_pitch_value is None:
    #     initial_pitch_value = euler_upper[1]
    #     print("Initial pitch value set to", initial_pitch_value)

    # if initial_roll_value is None:
    #     initial_roll_value = euler_upper[0]
    #     print("Initial roll value set to", initial_roll_value)

    # quat_upper_yaw = quaternion_from_euler(euler_upper[0] - initial_roll_value, euler_upper[1] - initial_pitch_value, euler_upper[2] - initial_yaw_value)
    
    
    quat_upper_yaw = quaternion_from_euler(euler_upper[0] , -(90* math.pi/180 + euler_upper[1]) , euler_upper[2] )
    ...
    br.sendTransform(t)
    print(math.floor(quat_upper_yaw[1] * 180 / math.pi), math.floor(quat_upper_yaw[2] * 180 / math.pi), initial_yaw_value, math.floor(rotating_base), i)
    # static_transform_publisher("boom", "laser", -1.245, 0, 0, 0, 0, 0)
    static_transform_publisher("boom", "laser", 20.392 ,0.63, 0.82, -90*math.pi/180, 6*math.pi/180, 180*math.pi/180); 
    # static_transform_publisher("boom", "laser", 20.392 ,0.63, 0.82, -90*math.pi/180, 0*math.pi/180, 0*math.pi/180); 

    static_transform_publisher("world", "odom", 0, 0, 0, 0, 0, 0)
