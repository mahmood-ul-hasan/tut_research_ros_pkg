...
//====================================================
  ...
  pcl::copyPointCloud(*cloud_filtered, cluster_cloud_indices, *extracted_cluster_cloud);
  pcl::copyPointCloud(*extracted_cluster_cloud, wall_inliers, *extracted_cloud_plane);


std::vector<int> plane_indices_in_filtered_cloud;
for (const auto& cluster_point_index : cluster_cloud_indices) {
    // Check if the point index is also in w_inliers, indicating it's part of the plane
    if (std::find(w_inliers->indices.begin(), w_inliers->indices.end(), cluster_point_index) != w_inliers->indices.end()) {
        plane_indices_in_filtered_cloud.push_back(cluster_point_index);
    }
}


pcl::PointCloud<pcl::PointXYZ>::Ptr extracted_plane_from_input_cloud(new pcl::PointCloud<pcl::PointXYZ>);
pcl::copyPointCloud(*cloud_filtered, plane_indices_in_filtered_cloud, *extracted_plane_from_input_cloud);
  ...
  viewer_frame_cloud.addPointCloud<pcl::PointXYZ>(extracted_plane_from_input_cloud, single_color_wall_cloud1, "extracted_frame_cloud"+ std::to_string(viewport_num), viewport_num);
