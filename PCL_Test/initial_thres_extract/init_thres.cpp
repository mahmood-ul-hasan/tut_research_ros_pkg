...
void fit_plane(pcl::PointCloud<PointT>::Ptr cloud_filterred, std::vector<float> coeffs_set[3], int num_plane, std::vector<double> dist_thres);

bool Const_X = false;
bool Const_Y = false;
bool Const_Z = false;

Eigen::VectorXf plane_coff; //coefficient for plane
...
bool G = false;      //ground plane 
bool W1 = false;   //wall plane1
bool W2 = false;   //wall plane2
bool W3 = false;   //wall plane3
bool DET_THRES = false;
#define NUM_PLANE 3
#define DIST_THRES 1.5
...
int main (int argc, char** argv)
{
    ...
    pcl::PointCloud<PointT>::Ptr filterred_cloud_copy (new pcl::PointCloud<PointT>);
    std::vector<double>max_dist_plane;
    std::vector<double>dist_thres;
    dist_thres.push_back(DIST_THRES);

    load_pcd_file(pcd_file, cloud);
    std::vector<int> indices;
    remove_nan_data(filterred_cloud, cloud, indices);  //remove the NaN data from the original cloud
    *filterred_cloud_copy = *filterred_cloud;
    std::vector<float>coefficients[4];
    fit_plane(filterred_cloud, coefficients, NUM_PLANE, dist_thres);
    for(int i=0; i < NUM_PLANE; i++){
        if(i == 0)
            plane_cloud = plane_points_ground;
        else if(i == 1)
            plane_cloud = plane_points_wall1;
        else if(i == 2)
            plane_cloud = plane_points_wall2;
        else if(i == 3)
            plane_cloud = plane_points_wall3;
        
        PointT ptPlane;
        plane_coff = Eigen::Vector4f(coefficients[i][0], coefficients[i][1], coefficients[i][2], coefficients[i][3]);
        ...
        ptPlane.x = plane_cloud->points[50].x;
        ptPlane.y = plane_cloud->points[50].y;
        ptPlane.z = plane_cloud->points[50].z;
        ...
        orthogonal_projection(plane_cloud, projected_points, plane_coff);
        ...
        double max_distance = *max_element(distances_cloud.begin(), distances_cloud.end());
        //std::cout << "Max Distance: " << max_distance << std::endl;
        max_dist_plane.push_back(max_distance);
    ...
    for(int i = 0; i < NUM_PLANE; i++)
        std::cout << "Max Distance for Plane_"<<i<<": " << max_dist_plane[i] << std::endl;
    DET_THRES = true;
    fit_plane(filterred_cloud_copy, coefficients, NUM_PLANE, max_dist_plane);
    return (0);
}
...
void fit_plane(pcl::PointCloud<PointT>::Ptr cloud_filterred, std::vector<float> coeffs_set[3], int num_plane, std::vector<double> dist_thres)
{
  ...
  std::vector<int> outliers_idxs;
  int i = 0;
  int thres_size;
  thres_size = dist_thres.size();   
  //Extract 4 planes
  while (i < num_plane)
  {
    ...
    if(thres_size == 1)
        seg.setDistanceThreshold(dist_thres[0]);
    else if(thres_size > 1)
        seg.setDistanceThreshold(dist_thres[i]);
    seg.setAxis(axis);
    ...
    if(DET_THRES == true)
        ss << "new_pcl" << i << ".pcd";
    else if(DET_THRES == false)
        ss << "old_pcl" << i << ".pcd";
	pcl::io::savePCDFileASCII(ss.str (), *cloud_p);

    // Create the filtering object
    extract.setNegative (true);
    extract.filter(outliers_idxs);
    extract.setNegative (true);
