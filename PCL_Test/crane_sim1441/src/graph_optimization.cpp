...
void graph_construction()
{
  ...
  node_array.clear();      //Clear the contents of the vector, if any.
...
void modified_pcd_file(pcl::PointCloud<PointT>::Ptr modified_cloud, std::string merged_file)
{
  ...
  *modified_cloud = *merge_cloud;
  pcl::io::savePCDFileASCII (merged_file, *merge_cloud);
  std::cerr << "Saved " << merge_cloud->points.size () << " data points to " <<merged_file<< std::endl;
}
...
{
  graph_slam->free_graph();
  GraphSLAM *raw = graph_slam.release();
  delete raw;

  filterred_idx.frame_id.clear();
  filterred_idx.min_index.clear();
  filterred_idx.max_index.clear();
  
  plane_idx_ground.frame_id.clear();
  plane_idx_ground.min_index.clear();
  plane_idx_ground.max_index.clear();
  
  plane_idx_wall1.frame_id.clear();
  plane_idx_wall1.min_index.clear();
  plane_idx_wall1.max_index.clear();
  
  plane_idx_wall2.frame_id.clear();
  plane_idx_wall2.min_index.clear();
  plane_idx_wall2.max_index.clear();
  
  plane_idx_wall3.frame_id.clear();
  plane_idx_wall3.min_index.clear();
  plane_idx_wall3.max_index.clear();
  
  rss_distances_ground.clear();
  rss_distances_wall1.clear();
  rss_distances_wall2.clear();
  rss_distances_wall3.clear();
}