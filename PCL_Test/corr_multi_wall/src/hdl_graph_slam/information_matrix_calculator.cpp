...
InformationMatrixCalculator::InformationMatrixCalculator(){ //ros::NodeHandle& nh) {
  use_const_inf_matrix = true; //nh.param<double>("use_const_inf_matrix", false);
  const_stddev_xy = 0.0008; //nh.param<double>("const_stddev_x", 0.5);
  const_stddev_z = 0.0007; 
  const_stddev_q1 = 0.01; //nh.param<double>("const_stddev_q", 0.1);
  const_stddev_q2 = 0.0001;
  const_stddev_q3 = 0.0001;

  var_gain_a = 20.0; //nh.param<double>("var_gain_a", 20.0);
  min_stddev_x = 0.1; //nh.param<double>("min_stddev_x", 0.1);
  max_stddev_x = 5.0; //nh.param<double>("max_stddev_x", 5.0);
  min_stddev_q = 0.05; //nh.param<double>("min_stddev_q", 0.05);
  max_stddev_q = 0.2; //nh.param<double>("max_stddev_q", 0.2);
  fitness_score_thresh = 0.5; //nh.param<double>("fitness_score_thresh", 0.5);
}
...
Eigen::MatrixXd InformationMatrixCalculator::calc_const_information_matrix() const {
  if(use_const_inf_matrix) {
    ...
    inf.topLeftCorner(2, 2).array() /= const_stddev_xy;
