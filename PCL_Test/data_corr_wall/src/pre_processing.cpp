...
//compute the frame wise rms error for wall plane in order to calculate the error vector
double wall_plane_rms_error_vertexi(Eigen::Isometry3d transform_mat,int vertexID)
{
  ...
  extract_frame_cloud(frame_cloud, plane_points_wall, plane_idx_wall, vertexID);
  trnasform_point_cloud(frame_cloud, transformed_cloud, transform_mat); 
  compute_distance(transformed_cloud, coeffs_wall, frame_pts_distances);
  //compute the rms distance
  for(size_t i = 0; i<(plane_idx_wall.max_index[vertexID-1] - plane_idx_wall.min_index[vertexID-1] + 1); i++)
    dist+= pow(frame_pts_distances[i],2);
  rms_dist_wall = sqrt(dist);
  
  //std::cout<<"F_ID:"<<vertexID<<" rms_dist diff wall plane:"<<rms_dist_wall-rms_distances_wall[vertexID-1]<<"\n";
