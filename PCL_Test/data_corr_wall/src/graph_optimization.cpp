...
#include <g2o/edge_se3_wall_plane.hpp>
...
void graph_construction(Eigen::VectorXf &floor_coeffs)
{
  ...
  //add wall_plane_node and wall_plane_edge to the graph
  wall_plane_node = graph_slam->add_plane_node(Eigen::Vector4d(coeffs_wall[0], coeffs_wall[1], coeffs_wall[2], coeffs_wall[3]));//0.0, 0.0, 1.0, 0.0));
  wall_plane_node->setFixed(true);
  Eigen::Vector4d measurement_wall(0,0,0,0);
  Eigen::Matrix3d information_wall = Eigen::Matrix3d::Identity();
  information_wall(0,0) = 1.0 / wall_edge_stddev;
  information_wall(1,1) = 0;//1.0 / wall_edge_stddev;
  information_wall(2,2) = 0;//1.0 / wall_edge_stddev;

  for(int i = 0; i < plane_idx_wall.frame_id.size(); i++)
  {
     if((plane_idx_wall.min_index[i] != -1) && (plane_idx_wall.max_index[i] != -1)){
       cur_node = node_array[i];
       auto edge2 = graph_slam->add_se3_wall_plane_edge(cur_node, wall_plane_node, measurement_wall, information_wall);
       //std::cout<<"Edge IDs:"<<edge2->vertices()[0]->id()<<" "<<edge2->vertices()[1]->id()<<std::endl;
     }
  }
}
