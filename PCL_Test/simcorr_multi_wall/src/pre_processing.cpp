...
{
  pcl::PointCloud<PointT>::Ptr cloud_filterred (new pcl::PointCloud<PointT>);
  ...
  load_pcd_file(pcd_file, cloud_original);
  remove_nan_data(cloud_filterred, cloud_original, indx);
  ...
  while (i < 4)
  {
    ...
    if(i==0)                    //ground plane; plane perpendicular to the z-axis
      ...
      axis << 1.0, 0.0, 0.0;
    else                        //plane perpendicular to the y-axis
      axis << 0.0, 1.0, 0.0;
