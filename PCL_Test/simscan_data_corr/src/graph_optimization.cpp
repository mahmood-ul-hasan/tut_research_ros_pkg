...
void graph_construction(Eigen::VectorXf &floor_coeffs)
{
  ...
  rel_pose.matrix() = x;
  std::cout << "Rotation: " << std::endl << rel_pose.rotation()<< std::endl;
  std::cout << "Matrix: " << std::endl << rel_pose.matrix()<< std::endl;
  //prev_node->setFixed(true);
  ...
  //information(1,1) = 1.0 / floor_edge_stddev;
  //information(2,2) = 1.0 / floor_edge_stddev;
  //std::cout<<"Floor Coeffs: "<<floor_coeffs<<std::endl;

  auto edge = graph_slam->add_se3_plane_edge(prev_node, floor_plane_node, measurement, information);
  ...
  for(int i = 1; i < plane_idx.frame_id.size(); i++)
  {
    ...
    auto edge2 = graph_slam->add_se3_plane_edge(cur_node, floor_plane_node, Eigen::Vector4d(0,0,0,0), information);
    //graph_slam->add_robust_kernel(edge2, floor_edge_robust_kernel, floor_edge_robust_kernel_size);
    prev_node = cur_node;
  }
 
  //Eigen::Matrix3d m = sen_pose.rotation();
  //Eigen::Vector3d v = sen_pose.translation();
...
double calc_globalRSS_after_optimization(pcl::PointCloud<PointT>::Ptr plane_pts, key_index plane_index, Eigen::VectorXf coeff)
{
