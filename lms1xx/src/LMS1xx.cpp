...
void LMS1xx::parseScanData(char* buffer, scanData* data)
{
  ...
  for (int i = 0; i < NumberChannels8Bit; i++)
  {
    ...
    tok = strtok(NULL, " "); //ScalingFactor
    // logDebug("ScalingFactor(2) : %s", tok);
    // std::cout << "ScalingFactor(2) : " << tok << std::endl;
    tok = strtok(NULL, " "); //ScalingOffset
    // logDebug("ScalingOffset(2) : %s", tok);
...
void LMS1xx::setAutostartMeasurement(/*bool isSetAuto*/)
{
  char buf[100];
