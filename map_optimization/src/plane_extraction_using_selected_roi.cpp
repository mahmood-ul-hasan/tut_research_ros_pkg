...
pcl::PointCloud<pcl::PointXYZ>::Ptr extract_roi_and_plane_segment(const pcl::PointCloud<pcl::PointXYZ>::Ptr& input_cloud, Eigen::Vector4f min_point, Eigen::Vector4f max_point)
{
...
pcl::SACSegmentation<pcl::PointXYZ> seg;
    ...
    pcl::PointCloud<pcl::PointXYZ>::Ptr wall_seg_cloud(new pcl::PointCloud<pcl::PointXYZ>);
...
pcl::visualization::PointCloudColorHandlerCustom<pcl::PointXYZ> single_color(input_cloud, 255, 255, 255);
viewer.addPointCloud<pcl::PointXYZ>(input_cloud, single_color, "original_cloud");
viewer.setPointCloudRenderingProperties(pcl::visualization::PCL_VISUALIZER_POINT_SIZE, 1, "original_cloud");

pcl::visualization::PointCloudColorHandlerCustom<pcl::PointXYZ> roi_color(roi_cloud, 255, 0, 0);
viewer.addPointCloud<pcl::PointXYZ>(roi_cloud, roi_color, "roi_cloud");
viewer.setPointCloudRenderingProperties(pcl::visualization::PCL_VISUALIZER_POINT_SIZE, 1, "roi_cloud");

pcl::visualization::PointCloudColorHandlerCustom<pcl::PointXYZ> wall_seg_color(roi_cloud, 0, 255, 0);
viewer.addPointCloud<pcl::PointXYZ>(wall_seg_cloud, wall_seg_color, "wall_seg_color");
...
int main(int argc, char** argv)
{
  ...
  std::string pcd_file = build_folder + "original_data_downsampled.pcd";
    ...
    if (pcl::io::loadPCDFile<pcl::PointXYZ>(pcd_file, *input_cloud) == -1)
    {
    ...
    Eigen::Vector4f min_point_g(-std::numeric_limits<float>::infinity(), -std::numeric_limits<float>::infinity(), -30, 1);
    Eigen::Vector4f max_point_g(std::numeric_limits<float>::infinity(),  std::numeric_limits<float>::infinity(), 0, 1);
    wall_seg_cloud = extract_roi_and_plane_segment(input_cloud, min_point_g, max_point_g);
    save_pcd_file = build_folder + "roi_ground_seg_cloud1.pcd";
    pcl::io::savePCDFileASCII(save_pcd_file, *wall_seg_cloud);  // Save the ROI cloud to a new PCD file


    Eigen::Vector4f min_point1(-std::numeric_limits<float>::infinity(), -60, -std::numeric_limits<float>::infinity(), 1);
    Eigen::Vector4f max_point1(std::numeric_limits<float>::infinity(), -40, std::numeric_limits<float>::infinity(), 1);
    wall_seg_cloud = extract_roi_and_plane_segment(input_cloud, min_point1, max_point1);
    save_pcd_file = build_folder + "roi_wall_seg_cloud1.pcd";
    pcl::io::savePCDFileASCII(save_pcd_file, *wall_seg_cloud);  // Save the ROI cloud to a new PCD file

    Eigen::Vector4f min_point2(-std::numeric_limits<float>::infinity(), 40, -std::numeric_limits<float>::infinity(), 1);
    Eigen::Vector4f max_point2(std::numeric_limits<float>::infinity(), 60, std::numeric_limits<float>::infinity(), 1);
    wall_seg_cloud = extract_roi_and_plane_segment(input_cloud, min_point2, max_point2);
    save_pcd_file = build_folder + "roi_wall_seg_cloud2.pcd";
    pcl::io::savePCDFileASCII(save_pcd_file, *wall_seg_cloud);  // Save the ROI cloud to a new PCD file


    // Define the Region of Interest (ROI)
    Eigen::Vector4f min_point(-70, -40, -std::numeric_limits<float>::infinity(), 1);
    Eigen::Vector4f max_point(-55.0, 40, std::numeric_limits<float>::infinity(), 1);
    wall_seg_cloud = extract_roi_and_plane_segment(input_cloud, min_point, max_point);
    save_pcd_file = build_folder + "roi_wall_seg_cloud3.pcd";
    pcl::io::savePCDFileASCII(save_pcd_file, *wall_seg_cloud);  // Save the ROI cloud to a new PCD file
