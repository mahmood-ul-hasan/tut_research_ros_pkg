...
void visensor_load_settings(const char *settings_file) {
    ...
    std::ifstream ifs_settings(visensor_settings_file_name.c_str()); //,ios::in|ios::binary|ios::ate);
    
    std::cout << "visensor_settings_file_name success -----"  << std::endl;
    ...
    for(int i = 3; i <= 15; i++) {
        ...
        std::getline(iss_line, token, ',');
        if(token[0] == 'l')
            visensor_cam_selection = 2;
        else if(token[0] == 'r')
            visensor_cam_selection = 1;
        else if(token[0] == 's')
            visensor_cam_selection = 0;
...
void visensor_save_current_settings() {
    ...
    f << "#" << endl
      << "m13,Manual Mode" << endl
      << modes_settings[12][0]
      << endl
      << modes_settings[12][1] << endl;
    if(modes_settings[12][2] == 0)
        f << "VGA" << endl;
    else
        f << "WVGA" << endl;
    ...
    f << imu_port_name << "," << VI_FIFO_matcher << endl
      << "#" << endl
      << "IMU-acc-bias" << endl;
...
int check_img(int cam_no, unsigned char *pImg, unsigned char *pImgPass) {
    ...
    if(!visensor_resolution_status)
        *pImgPass = (pImg[IMG_SIZE_VGA + 0] == 0xFF && pImg[IMG_SIZE_VGA + 1] == 0x00 && pImg[IMG_SIZE_VGA + 2] == 0xFE && pImg[IMG_SIZE_VGA + 3] == 0x01);
    else
        *pImgPass = (pImg[IMG_SIZE_WVGA + 0] == 0xFF && pImg[IMG_SIZE_WVGA + 1] == 0x00 && pImg[IMG_SIZE_WVGA + 2] == 0xFE && pImg[IMG_SIZE_WVGA + 3] == 0x01);
    if(*pImgPass == 0) {
...
float visensor_get_hardware_fps() {
    if(gFPS == 54) {
        if(!visensor_resolution_status)
            return 45700.0f / (current_HB + 640.0f);
        else
            return 45700.0f / (current_HB + 752.0f);
    } else if(gFPS == 27) {
        if(!visensor_resolution_status)
            return 0.5f * 45700.0f / (current_HB + 640.0f);
        else
            return 0.5f * 45700.0f / (current_HB + 752.0f);
    }
}
...
int visensor_Start_IMU() {
    ...
    cout << "visensor_Start_IMU  fd: " << fd << endl;
