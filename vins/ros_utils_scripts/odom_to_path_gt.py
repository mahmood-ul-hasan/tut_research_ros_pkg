...
class path_pub():
    def __init__(self):
        rospy.init_node('odom_to_path_gt', anonymous=True)

        self.append_rate = rospy.get_param("~append_rate", 25)

        self.gt_path_pub = rospy.Publisher("/camera/odom/sample_world_frame_path", Path, queue_size=2)
        self.gt_poses = rospy.Subscriber('/camera/odom/sample_world_frame', Odometry, self.gtcallback)
