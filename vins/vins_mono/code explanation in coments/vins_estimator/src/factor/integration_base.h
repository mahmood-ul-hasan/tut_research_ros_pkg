...
class IntegrationBase {
  public:
    ...
    /*
    repropagate () new bias recalculates the pre-integration
    The Bias will be updated during the optimization process, and the pre-integration needs to be recalculated according to the new bias
This function is located in the integration_base.h file, and its function is to recalculate the value of pre-integration according to the new acceleration and angular velocity deviation

repropagate(
const Eigen::Vector3d &_linearized_ba, //New acceleration deviation
const Eigen::Vector3d &_linearized_bg)//New angular velocity deviation
    */
    ...
    /*
    This function is located in the integration_base.h file, and its role is to perform median integration, which is more accurate than direct integration

In the IMU pre-integration, the median integral is used to recurse the linearized recursive equation of Jacobian and Covariance to construct the error,
and obtain the recursive formula of Jacobian and Covariance -> Paper Equations 9, 10, 11

    midPointIntegration(
    double _dt, //time interval
    const Eigen::Vector3d &_acc_0, // last measured acceleration
    const Eigen::Vector3d &_gyr_0, // last measured angular velocity
    const Eigen::Vector3d &_acc_1, //acceleration measured this time
    const Eigen::Vector3d &_gyr_1, //measure angular velocity this time
    const Eigen::Vector3d &delta_p,//Displacement relative to the initial reference system
    const Eigen::Quaterniond &delta_q, //The attitude relative to the initial frame of reference
    const Eigen::Vector3d &delta_v,//The speed relative to the initial reference frame
    const Eigen::Vector3d &linearized_ba, //acceleration deviation relative to the initial reference frame
    const Eigen::Vector3d &linearized_bg,//The angular velocity deviation relative to the initial reference frame
    Eigen::Vector3d &result_delta_p, //calculation result of attitude change
    Eigen::Quaterniond &result_delta_q, //Calculation result of position change
    Eigen::Vector3d &result_delta_v,//Calculation result of velocity variation
    Eigen::Vector3d &result_linearized_ba, //acceleration deviation calculation result
    Eigen::Vector3d &result_linearized_bg, //Calculation result of angular velocity deviation
    bool update_jacobian)//whether to update Jacobian
    */
    ...
    propagate(
    double _dt, //time interval
    const Eigen::Vector3d &_acc_1, //linear acceleration
    const Eigen::Vector3d &_gyr_1) //angular velocity

    This function  role is to integrate the amount of change measured by the IMU between two keyframes:

    Rotation delta_q Velocity delta_v Displacement delta_p
    Acceleration's biaslinearized_ba Gyroscope's Bias linearized_bg
    At the same time maintain and update the Jacobian and Covariance of the pre-integration, and calculate the necessary parameters for optimization
    ...
    */
    ...
    This function is located in the integration_base.h file, and its function is to calculate the residual between the i-th frame and the j-th frame
    It is Eq:34 from paper
    evaluate(
    const Eigen::Vector3d &Pi, //position of frame i
    const Eigen::Quaterniond &Qi, // attitude of frame i
    const Eigen::Vector3d &Vi, //speed of frame i
    const Eigen::Vector3d &Bai, //acceleration deviation of frame i
    const Eigen::Vector3d &Bgi, // angular velocity deviation of frame i
    const Eigen::Vector3d &Pj, // jth frame position
    const Eigen::Quaterniond &Qj, //J frame attitude
    const Eigen::Vector3d &Vj, // jth frame speed
    const Eigen::Vector3d &Baj, //Acceleration deviation of frame j
    const Eigen::Vector3d &Bgj)//J frame angular velocity deviation
