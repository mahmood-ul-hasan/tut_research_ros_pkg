...
void save_data(const std::string& dst_directory, const nav_msgs::OdometryConstPtr& data) {
  ...
  // std::cout << dst_directory << "/" << data->header.stamp.sec << "_" << boost::format("%02d") % data->header.stamp.nsec << ".odom" << std::endl;
...
public:
  ...
  size_t size() const {
    ...
    // std::cout << dst_directory << "  dst_directory" << std::endl;
    return queue_size;
  }
  ...
  void push(const T& data) {
    ...
    // std::cout << queue.front() << "queue" << std::endl;
...
private:
  ...
  void points_callback(const sensor_msgs::PointCloud2ConstPtr& points_msg) {
    ...
    // std::cout << *points_msg << " points_msg" << std::endl;
  ...
  void odometry_callback(const nav_msgs::OdometryConstPtr& odometry_msg) {
    ...
    // std::cout << *odometry_msg << " odometry_msg " << std::endl;
